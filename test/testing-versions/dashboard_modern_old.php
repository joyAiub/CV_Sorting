    ...
    <script type="text/babel">
        ...
        function Dashboard() {
            ...
            const mergeWithDefaults = useCallback((sourceCols) => {
                if (!sourceCols) return DEFAULT_COLUMNS;
                ...
                const initColumns = async () => {
                    ...
                    if (saved) {
                        try {
                            ...
                            // Check if local differs from global
                            if (serverGlobal) {
                    ...
                    setColumns(mergeWithDefaults(serverGlobal));
                };
                initColumns();
            }, [mergeWithDefaults]);
            ...
            const applyGlobal = () => {
                ...
                Swal.fire({ icon: 'success', title: 'Layout Updated', text: 'Global default layout has been applied.', timer: 2000, showConfirmButton: false });
            };
            ...
            useEffect(() => {
                const pollInterval = setInterval(() => {
                    ...
                    if (!editingRowId && !isSelectionActive && !isAnyModalOpen) {
            ...
            const toggleColumn = (id) => {
                setColumns(prev => {
                    ...
                    localStorage.setItem('modern_dashboard_columns_v2', JSON.stringify(next));
                    return next;
                });
            };
                ...
                <div className="flex flex-col h-full bg-slate-50 relative text-slate-800">
                    ...
                    {showSettings && (
                        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-6">
                            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-row max-h-[90vh] animate-in zoom-in duration-200 overflow-hidden">
                                {/* Left Side: Templates */}
                                <div className="w-1/3 border-r border-slate-100 bg-slate-50 p-6 flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
                                        {Object.keys(templates).length === 0 ? (
                                            ...
                                            <div key={name} className="flex items-center justify-between group bg-white p-2.5 rounded-xl border border-slate-200 hover:border-indigo-600 transition-all cursor-pointer shadow-sm" onClick={() => loadTemplate(name)}>
                                    ...
                                    <div className="pt-4 border-t border-slate-200 flex flex-col gap-2">
                                        <input 
                                            ...
                                            className="w-full px-3 py-2 text-[11px] bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-600/20"
                                            value={newTemplateName}
                                            onChange={(e) => setNewTemplateName(e.target.value)}
                                        />
                                        <button onClick={() => saveTemplate(false)} className="w-full bg-indigo-600 text-white py-2 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-md">
                                            Save Current
                                        </button>
                                        {canManageGlobalLayouts && (
                                            <button onClick={() => saveTemplate(true)} className="w-full bg-emerald-600 text-white py-2 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-md mt-1">
                                                Save Globally (All Users)
                                            </button>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 flex flex-col">
                                    <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-white">
                                        <h2 className="font-black text-slate-800 uppercase tracking-tight text-sm">Customize Columns</h2>
                                        <button onClick={() => setShowSettings(false)} className="text-slate-400 hover:text-slate-600 transition-colors">
                                            <Icon name="close" />
                                        </button>
                                    </div>
                                    <div className="p-4 overflow-y-auto flex-1 flex flex-col gap-2 bg-white">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Visibility & Sequence</p>
                                        {columns.map((col, idx) => (
                                            <div key={col.id} className="flex items-center gap-3 bg-slate-50 p-2 rounded-xl border border-slate-100 group">
                                                <div className="flex flex-col gap-1">
                                                    <button onClick={() => moveColumn(col.id, -1)} disabled={idx === 0} className="text-slate-300 hover:text-indigo-600 disabled:opacity-0 transition-all"><Icon name="expand_less" className="w-4 h-4" /></button>
                                                    <button onClick={() => moveColumn(col.id, 1)} disabled={idx === columns.length - 1} className="text-slate-300 hover:text-indigo-600 disabled:opacity-0 transition-all"><Icon name="expand_more" className="w-4 h-4" /></button>
                                                </div>
                                                <div className="flex-1 flex items-center gap-3">
                                                    <input 
                                                        ...
                                                        disabled={col.sticky}
                                                    />
                                                    <span className={`text-[11px] font-bold ${col.hidden ? 'text-slate-400' : 'text-slate-700'}`}>{col.label}</span>
                                                </div>
                                                {col.sticky && <span className="text-[8px] font-black text-indigo-500 uppercase bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-100">Sticky</span>}
                                            </div>
                                        ))}
                                    </div>
                                    <div className="p-4 border-t border-slate-100 flex justify-end bg-white">
                                        <button onClick={() => setShowSettings(false)} className="bg-slate-900 text-white px-8 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest shadow-lg hover:bg-slate-800 transition-all">Done</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 relative">
                            ...
                            >
                                <div className="flex flex-col items-end mr-1">
                                    <span className="text-[11px] font-bold text-slate-200 leading-tight">{isPublic ? 'Public' : userName}</span>
                                    <span className="text-[9px] text-slate-500 font-medium leading-tight uppercase tracking-widest">{isPublic ? 'View' : userRole}</span>
                                </div>
                                        ...
                                        {activeColumns.map(col => (
                                            ...
                                            >
                                                <div className="line-clamp-2-custom">
                                                    <CellContent id={col.id} candidate={c} sl={(pagination.current_page - 1) * 50 + i + 1} expanded={expandedAll} openPdf={openPdf} jdId={jdId} onToggle={handleToggle} editingRowId={editingRowId} setEditingRowId={setEditingRowId} />
                                                </div>
                                            </td>
                                        ))}
                                    </tr>
        ...
        function CellContent({ id, candidate, sl, expanded, openPdf, jdId, onToggle, editingRowId, setEditingRowId }) {
