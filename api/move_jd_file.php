...
$source_dir = getRpaConfigValue($conn, 'N8NJDPATH', "C:\\Users\\admin\\.n8n-files\\JD");
$dest_dir = ""; // Will be resolved dynamically

$jd_id = $_GET['jd_id'] ?? '';

// Resolve dynamic path using helper
if (!empty($jd_id)) {
    ...
    if (!empty($task_no)) {
        $dest_dir = get_processed_path($conn, $task_no, "JD");
    }
}
...
if (empty($dest_dir)) {
    ...
    $dest_dir = $base_processed . DIRECTORY_SEPARATOR . "JD";
}
...
if (empty($filename)) {
    echo json_encode(["status" => "error", "message" => "filename parameter is mandatory. Use '-' to move all files."]);
    exit;
}

if ($filename === '-') {
    // Move all files if filename is '-'
    $files_moved = 0;
    $errors = [];
    ...
    $status = "success";
    if ($files_moved === 0) {
        $msg = "No files found to move.";
    } else {
        $msg = "$files_moved JD files moved successfully.";
    }
    if (count($errors) > 0) {
    ...
    if (file_exists($source_path)) {
        if (rename($source_path, $dest_path)) {
            ob_clean();
            echo json_encode(["status" => "success", "message" => "$target_name moved successfully."]);
        }
        else {
            ob_clean();
            echo json_encode(["status" => "error", "message" => "Failed to move $filename."]);
        }
    }
    else {
        ...
        if (file_exists($dest_path)) {
            ob_clean();
            echo json_encode(["status" => "success", "message" => "$target_name is already in destination."]);
        } else {
            ob_clean();
