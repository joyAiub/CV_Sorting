...
window.allJobsData = Array.isArray(window.allJobsData) ? window.allJobsData : [];
...
window.applyJobFilters = function() {
    ...
    const filtered = window.allJobsData.filter(job => {
        ...
        if (viewMode === 'my') {
            ...
            // Check if user is the creator or if it's explicitly shared with them
            const allowedViewers = String(job.allowed_viewers || '').toLowerCase().split(',').map(s => s.trim());
            const isShared = allowedViewers.includes(loggedInUser);
            
            matchesViewMode = (loggedInUser === jobCreator || isShared);
        }

        // Test Task Visibility Logic
        if (matchesViewMode && viewMode === 'all' && job.type === 'test') {
            const loggedInUser = (window.currentUser?.id || '').toLowerCase().trim();
            const jobCreator = (job.created_by || '').toLowerCase().trim();
            const isRoot = (window.currentUser?.id && window.currentUser.id === window.rootAdminId);
            // Hide test tasks from 'All Tasks' unless they are root admin or the creator
            if (!isRoot && loggedInUser !== jobCreator) {
        ...
        if (result.status === 'success') {
            ...
            result.data.forEach(user => {
                ...
                if (isSelf) {
                    ...
                    const buttons = [];
                                    // 1. Permissions Button — role-hierarchy aware
                    // Root: all | Super-admin: all except other super-admins
                    // Admin: user/sub-admin only | User with perm: user only
                    const callerRole = window.currentUserRole || 'user';
                    let canManagePerms = false;
                    if (window.canManageRoles) {
                        if (isRootAdmin) {
                            canManagePerms = true;
                        } else if (window.isSuperAdmin) {
                            canManagePerms = !isSuperAdminUser;
                        } else if (callerRole === 'admin') {
                            canManagePerms = (user.role === 'user' || user.role === 'sub-admin' || user.role === 'sub_admin');
                        } else {
                            canManagePerms = (user.role === 'user');
                        }
                    }
                    if (canManagePerms) {
...
function renderJobList(data) {
    ...
    data.forEach((job, index) => {
        ...
        const testBadge = job.type === 'test' ? `<span class="badge-jd" style="margin-left: 6px; font-size: 0.65rem; padding: 1px 5px; background: #fff5f5; color: #e11d48; border-color: #fecdd3; font-weight: 800; display: inline-flex; align-items: center; gap: 2px;"><span class="material-icons" style="font-size: 11px;">bug_report</span>TEST</span>` : '';
        const jobTitleHtml = `
        ...
        if (window.isTaskManagementPage) {
            // Check if current user is allowed to manage permissions
            const isCreator = (window.currentUser?.id && String(window.currentUser.id).toLowerCase().trim() === String(job.created_by).toLowerCase().trim());
            const isAdmin = (window.currentUser?.id && window.currentUser.id === window.rootAdminId) || 
                            window.isSuperAdmin || 
                            (window.currentUserPermissions && (window.currentUserPermissions['manage_tasks'] === true || window.currentUserPermissions['manage_tasks'] === 'true' || window.currentUserPermissions['manage_tasks'] === 1));
            
            let shareBtnHtml = '';
            if (isCreator || isAdmin) {
                // Escape job title for inline JS safely
                const safeTitle = job.job_title.replace(/'/g, "\\'").replace(/"/g, '&quot;');
                const safeAllowed = (job.allowed_viewers || '').replace(/'/g, "\\'").replace(/"/g, '&quot;');
                shareBtnHtml = `
                    <button onclick="window.openTaskSharing('${job.jd_id}', '${safeTitle}', '${safeAllowed}')" style="width: 26px; height: 26px; background: #ecfdf5; color: #059669; border: 1px solid #a7f3d0; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center;" title="Grant Task Permission">
            ...
            actionHtml = `
                <td style="text-align: center; padding: 6px 0;">
                    <div style="display: inline-flex; align-items: center; justify-content: center;">
                        <button onclick="triggerScreeningPerTask('${job.jd_id}', '${job.task_no}')" style="width: 28px; height: 28px; background: #dbeafe; color: #2563eb; border: 1px solid #93c5fd; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center;" title="Trigger AI Screening for this task">
            ...
            tr.innerHTML = `
                <td style="font-weight: 600; color: #94a3b8; text-align: center;">${index + 1}</td>
                <td style="text-align: left; padding-left: 15px;">${jobTitleHtml}</td>
                ...
                <td style="font-size: 0.75rem; color: #475569; line-height: 1.4; max-height: 4.2em; overflow-y: auto; white-space: normal; word-break: break-word; padding: 2px 0; scrollbar-width: thin;">
                    ${job.prompt_text || '—'}
                </td>
                ...
                <td style="font-weight: 700; color: #1d4ed8;">${job.total_candidate || 0}</td>
...
// --- Trigger AI Screening for a Specific Task ---
window.triggerScreeningPerTask = async function(jdId, taskNo) {
    if (!jdId || !taskNo) {
        alert('Missing JD ID or Task No');
        return;
    }

    if (!confirm(`Trigger AI screening for Task ${taskNo}?`)) return;

    try {
        const res = await fetch(getApiUrl('rpa_config_api.php?action=get&key=n8n_webhook_url'));
        const data = await res.json();
        const webhookUrl = data.value || '';

        if (!webhookUrl) {
            alert('n8n webhook URL not configured. Check RPA Config.');
            return;
        }

        const screeningUrl = `${webhookUrl}?JD_ID=${encodeURIComponent(jdId)}&TASKNO=${encodeURIComponent(taskNo)}`;

        const curlRes = await fetch(screeningUrl, { method: 'POST' });
        if (curlRes.ok) {
            alert(`✓ Screening triggered for ${taskNo}`);
        } else {
            alert(`Error: HTTP ${curlRes.status}`);
        }
    } catch (err) {
        console.error('Error triggering screening:', err);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addJobBtn) {
        addJobBtn.addEventListener('click', async () => {
            ...
            const isTestCheckbox = document.getElementById('publicIsTestTask');
            const type = isTestCheckbox && isTestCheckbox.checked ? 'test' : '';
...
window.addEventListener('click', function (event) {
    ...
    const profile = document.querySelector('.user-profile-nav');
...
function initTaskViewMode() {
    ...
    // By default, always show 'my' tasks first as requested
...
// --- Task Access Permissions Management ---
window.currentSharingViewers = [];
window.sharingSearchTimeout = null;

window.openTaskSharing = function(jdId, title, allowedViewersStr) {
    const jdIdInput = document.getElementById('sharingTaskJdId');
    const taskTitleDiv = document.getElementById('sharingTaskTitle');
    const allowedListDiv = document.getElementById('sharingAllowedList');
    const modal = document.getElementById('taskSharingModal');
    const searchInput = document.getElementById('sharingEmpSearch');
    const suggestions = document.getElementById('sharingSuggestions');

    if (!modal) return;

    if (jdIdInput) jdIdInput.value = jdId;
    if (taskTitleDiv) taskTitleDiv.innerHTML = `${title} <span class="badge-jd" style="margin-left:8px;">${jdId}</span>`;
    ...
    // Parse the allowed viewers string into an array
    const viewers = String(allowedViewersStr || '').split(',').map(s => s.trim()).filter(Boolean);
    window.currentSharingViewers = viewers;

    // Open Modal
    modal.style.display = 'flex';

    // Render List
    window.renderSharingAllowedList();
};

window.closeTaskSharingModal = function() {
    const modal = document.getElementById('taskSharingModal');
    if (modal) modal.style.display = 'none';
};

window.searchSharingEmployees = function(query) {
    const suggestions = document.getElementById('sharingSuggestions');
    if (!suggestions) return;
    ...
    clearTimeout(window.sharingSearchTimeout);
    window.sharingSearchTimeout = setTimeout(async () => {
        try {
            const response = await fetch(getApiUrl(`employee_api.php?action=list&search=${encodeURIComponent(query)}&t=${Date.now()}`));
            const result = await response.json();
            ...
            if (result.status === 'success' && result.data.length > 0) {
                suggestions.innerHTML = result.data.slice(0, 10).map(emp => `
                    <div class="suggestion-item" onclick="window.selectSharingEmployee('${emp.employee_id}', '${emp.full_name.replace(/'/g, "\\'")}')" style="padding: 10px; cursor: pointer; border-bottom: 1px solid #f1f5f9;">
                        <div style="display: flex; justify-content: space-between;">
                            <span style="font-weight: 600; font-size: 0.85rem; color: #1e293b;">${emp.full_name}</span>
                            <span style="font-size: 0.8rem; color: #64748b; font-weight: 500;">ID: ${emp.employee_id}</span>
                        </div>
                        <div style="font-size: 0.75rem; color: #059669; font-weight: 500; margin-top: 2px;">${emp.designation} (${emp.department})</div>
                    </div>
                `).join('');
                suggestions.style.display = 'block';
            } else {
                suggestions.innerHTML = '<div style="padding: 10px; color: #64748b; font-size: 0.8rem; background: white;">No matching employee found</div>';
                suggestions.style.display = 'block';
            }
        } catch (e) {
            console.error(e);
...
window.selectSharingEmployee = function(id, name) {
    const searchInput = document.getElementById('sharingEmpSearch');
    const suggestions = document.getElementById('sharingSuggestions');
    if (searchInput) searchInput.value = `${name} (${id})`;
    if (suggestions) {
...
window.addSharingUser = function() {
    const searchInput = document.getElementById('sharingEmpSearch');
    if (!searchInput) return;

    const val = searchInput.value;
    // Extract ID inside parenthesis, or fallback to the whole trimmed value
    const match = val.match(/\(([^)]+)\)/);
    const userId = (match ? match[1] : val).trim();
    ...
    if (window.currentSharingViewers.includes(userId)) {
        alert("This user already has access to this task!");
        return;
    }

    window.currentSharingViewers.push(userId);
    searchInput.value = '';
    window.renderSharingAllowedList();
};

window.removeSharingUser = function(userId) {
    window.currentSharingViewers = window.currentSharingViewers.filter(id => id !== userId);
    window.renderSharingAllowedList();
};

window.renderSharingAllowedList = async function() {
    const listDiv = document.getElementById('sharingAllowedList');
    if (!listDiv) return;

    if (window.currentSharingViewers.length === 0) {
        listDiv.innerHTML = '<div style="padding: 20px; text-align: center; color: #94a3b8; font-size: 0.85rem; border: 2px dashed #e2e8f0; border-radius: 12px; background: #f8fafc;">This task is private. Only the creator and admins can view it.</div>';
        return;
    }

    listDiv.innerHTML = '<div style="padding: 10px; text-align: center; color: #64748b; font-size: 0.85rem;">Loading authorized users...</div>';

    try {
        // Fetch all employee details to show names and designations beautifully
        const items = await Promise.all(window.currentSharingViewers.map(async (userId) => {
            try {
                const response = await fetch(getApiUrl(`employee_api.php?action=list&search=${encodeURIComponent(userId)}&t=${Date.now()}`));
                const result = await response.json();
                if (result.status === 'success' && result.data.length > 0) {
                    // Match exact ID
                    const emp = result.data.find(e => String(e.employee_id) === String(userId)) || result.data[0];
                    return { id: userId, name: emp.full_name, designation: emp.designation, department: emp.department };
                }
            } catch (e) { console.error(e); }
            return { id: userId, name: 'Employee', designation: 'Unknown Designation', department: 'N/A' };
        }));

        listDiv.innerHTML = items.map(emp => `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; transition: all 0.2s;">
                <div style="display: flex; flex-direction: column; gap: 2px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <span style="font-weight: 700; font-size: 0.9rem; color: #1e293b;">${emp.name}</span>
                        <span style="background: #e2e8f0; color: #475569; font-size: 0.7rem; font-weight: 700; padding: 1px 6px; border-radius: 20px;">ID: ${emp.id}</span>
                    </div>
                    <div style="font-size: 0.75rem; color: #64748b; font-weight: 500;">${emp.designation} <span style="color:#cbd5e1; margin:0 4px;">|</span> ${emp.department}</div>
                </div>
                <button onclick="window.removeSharingUser('${emp.id}')" style="background: #fef2f2; color: #ef4444; border: 1px solid #fee2e2; width: 28px; height: 28px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;" onmouseover="this.style.background='#fee2e2'" onmouseout="this.style.background='#fef2f2'" title="Revoke Access">
                    <span class="material-icons" style="font-size: 16px;">close</span>
                </button>
            </div>
        `).join('');
    } catch (e) {
        listDiv.innerHTML = '<div style="padding: 20px; text-align: center; color: #ef4444; font-size: 0.85rem;">Failed to load users details.</div>';
    }
};

window.saveTaskSharing = async function() {
    const jdIdInput = document.getElementById('sharingTaskJdId');
    if (!jdIdInput || !jdIdInput.value) return;

    const jdId = jdIdInput.value;
    const allowedStr = window.currentSharingViewers.join(',');

    try {
        const response = await fetch(getApiUrl(`job_api.php?action=share&t=${Date.now()}`), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                jd_id: jdId,
                allowed_viewers: allowedStr
            })
        });
        ...
        if (result.status === 'success') {
            ...
            window.closeTaskSharingModal();
            // Refresh table locally to reflect the update immediately
            if (typeof loadJobList === 'function') await loadJobList();
        } else {
